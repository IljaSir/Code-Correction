...
// так как ниже встречаются обозначения CHAR и UINT, которых по умолчанию компилятор не понимает, обозначим их тут
...
#define CALC_PAGE_KEY( Addr, Color )	(  (Color) + (Addr) << 8 ) //тут должно быть *(Addr)
...
void PageStrgInit()
{
	memset( PageStrg, 0, sizeof(&PageStrg) ); // тут должно быть sizeof(PageStrg)
}
...
PageDesc* PageInit( void* ptr, UINT color )
{
    ...
    if( pg )
        PAGE_INIT(&pg, ptr, color); // тут должно быть PAGE_INIT(pg, ptr, color);
    else
...
void PageDump()
{
	...
	while( color <= PG_COLOR_RED )
	{
		...
		for( PageDesc* Pg = PageStrg[++color]; Pg != NULL; Pg = Pg->next ) // ++color выдаст ошибку, т.к. red - наибольший цвет
		{
			if( Pg->uAddr = NULL )
				continue;
